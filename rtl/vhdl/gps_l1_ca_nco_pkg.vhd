...
package gps_l1_ca_nco_pkg is
  function lo_cos_q15(phase_idx : unsigned(9 downto 0)) return signed;
  function lo_sin_q15(phase_idx : unsigned(9 downto 0)) return signed;
end package;

package body gps_l1_ca_nco_pkg is
  type q15_quarter_lut_t is array (0 to 256) of integer;
  constant C_COS_Q15_QUARTER_LUT : q15_quarter_lut_t := (
  ...
  function cos_q15_from_idx(idx : integer) return signed is
    variable n_i   : integer;
    variable quad_i: integer;
    variable off_i : integer;
  ...
  begin
    ...
    case quad_i is
      when 0 =>
        val_i := C_COS_Q15_QUARTER_LUT(off_i);
      when 1 =>
        val_i := -C_COS_Q15_QUARTER_LUT(256 - off_i);
      when 2 =>
        val_i := -C_COS_Q15_QUARTER_LUT(off_i);
      when others =>
        val_i := C_COS_Q15_QUARTER_LUT(256 - off_i);
    ...
    return to_signed(val_i, 16);
  end function;

  function lo_cos_q15(phase_idx : unsigned(9 downto 0)) return signed is
  begin
    return cos_q15_from_idx(to_integer(phase_idx));
  end function;

  function lo_sin_q15(phase_idx : unsigned(9 downto 0)) return signed is
  begin
    return cos_q15_from_idx(256 - to_integer(phase_idx));
  end function;
end package body;
